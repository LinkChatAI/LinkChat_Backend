...
await check('GET /api/auth/google → 302 to Google', async () => {
  const res = await fetch(`${BASE}/api/auth/google?returnTo=/`, { redirect: 'manual' });
  if (res.status !== 302) throw new Error(`expected 302, got ${res.status}`);
  const loc = res.headers.get('location') || '';
  if (!loc.includes('accounts.google.com')) throw new Error(`unexpected redirect: ${loc.slice(0, 80)}`);
});
